...
(function($) {
  ...
  $.fn.jqmTooltip = function(options) {
    ...
    function showTooltip(tooltip) {
      hideTooltip('all');
      //Only allow one tooltip at a time (AC)
    ...
    return this.each(function() {
      ...
      var $this = $(this), cssStyles = {}, tooltipID = 'tooltip-' + Math.floor(Math.random() * 1000), overlay, overlayWidth, overlayLeft, overlayOffset, element, elemOffset, windowHeight, windowWidth;
      ...
      overlay.html(settings.text).attr('id', tooltipID).css('display', 'none').append('<a href="#" class="close"></div>').find('.close').bind('tap', function(e) {
        e.preventDefault();
      ...
      $this.bind('tap.jqmTooltip', function(e) {
        ...
        if (overlay.hasClass('active')) {
          ...
          // position overlay horizontally based on trigger element and screen
          // width
